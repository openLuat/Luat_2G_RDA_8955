...
-- RSSI BSSS Base station signal strength，基站信号强度。
---[[
local function enCellInfo(s)
    local ct, cnt, ret = {}, 0, ""
    log.info("lbsLoc.enCellInfo:", s)
    for mcc, mnc, lac, ci, rssi in s:gmatch("(%d+)%.(%d+)%.(%d+)%.(%d+)%.(%d+);") do
        mcc, mnc, lac, ci, rssi = tonumber(mcc), tonumber(mnc), tonumber(lac), tonumber(ci), (tonumber(rssi) > 31) and 31 or tonumber(rssi)
        local mate = false
        for i = 1, #ct do
            if ct[i][1] == lac and ct[i][2] == mcc and ct[i][3] == mnc then
                ct[i][4] = ct[i][4] + 1
                table.insert(ct[i], rssi)
                table.insert(ct[i], ci)
                mate = true
                break
            end
        end
        if not mate then table.insert(ct, {lac, mcc, mnc, 0, rssi, ci}) end
    end
    for i = 1, #ct do
        ct[i][4] = bit.bor(bit.lshift(ct[i][4], 5), ct[i][5]); table.remove(ct[i], 5)
        ret = ret .. pack.pack(">HHb", ct[i][1], ct[i][2], ct[i][3])
        for j = 4, #ct[i], 2 do
            ret = ret .. pack.pack(">bH", ct[i][j], ct[i][j + 1])
        end
    end
    return string.char(#ct) .. ret
end
--]]
--[[
local function enCellInfo(s)
local ret, t, mcc, mnc, lac, ci, rssi, k, v, m, n, cntrssi = "", {}
log.info("lbsLoc.enCellInfo2", s)
